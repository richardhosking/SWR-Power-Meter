...
/* The controller object for the SSD1305 OLED display
 Inherits from Adafruit_GrayOLED Class - functions including 
...
class Display : public Adafruit_GrayOLED {
public:
  Display(uint16_t w, uint16_t h, TwoWire *twi = &Wire,
                   int8_t rst_pin = -1, uint32_t preclk = 400000,
                   uint32_t postclk = 100000);
  Display(uint16_t w, uint16_t h, int8_t mosi_pin, int8_t sclk_pin,
                   int8_t dc_pin, int8_t rst_pin, int8_t cs_pin);
  Display(uint16_t w, uint16_t h, SPIClass *spi, int8_t dc_pin,
  ...
  void sleep();
  void wake();
  void drawRectangle(int16_t startx, int16_t starty, int16_t endx, int16_t endy, uint16_t color);
